...
simdatG=simulateDate(sampleSize=sampleSize, numPred=numPred, overlapL=level, seed.num=d, rho=0, treatEff=2)
  ...
  modelType="lassoSep" ###seperate adaptive lasso on the propensity and prediction models
  ...
  estimate.out=NULL
  estimate.out=pencompLasso(dataOR=simdat, data=simdat, varList=varList, propenVarList=propenVarList,
 ...
 if( typeof(estimate.out) == "list" ){ ###if output is list, should be right
  ...
  for(ind in 1:numT){
   tryCatch (
   {
    ...
    mulResult = pencompLasso(dataOR=simdat, data=bootSample, varList=varList, propenVarList=propenVarList,
                 outcomeVarList0=outcomeVarList0, outcomeVarList1=outcomeVarList1,
                 treat.varname=treat.varname, outcome.varname=outcome.varname ) 
