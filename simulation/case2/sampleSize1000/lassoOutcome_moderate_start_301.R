...
source(paste0(DIREC, funLoc, "LassoOutcome.R"))
...
simdatG=simulateDate(sampleSize=sampleSize, numPred=numPred, overlapL=level, seed.num=d, rho=0, treatEff=2)
  ...
  estimate.out=NULL
  estimate.out=pencompLassoOutcome(dataOR=simdat, data=simdat, varList=varList, propenVarList=propenVarList,
  ...
  for(ind in 1:numT){
   tryCatch (
   {
    ...
    mulResult = pencompLassoOutcome(dataOR=simdat, data=bootSample, varList=varList, propenVarList=propenVarList,
                                    outcomeVarList0=outcomeVarList0, outcomeVarList1=outcomeVarList1,
                                    treat.varname=treat.varname, outcome.varname=outcome.varname ) 
