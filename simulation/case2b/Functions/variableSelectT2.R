...
######variable selections using outcome adaptive lasso ######################
expit = function(x){ 
  pr = ( exp(x) / (1+exp(x)) ) 
  return(pr)
}
...
outcomeAdapLasso=function(Data, varList, treat.varname, outcome.varname, threshold=10^-8) {
  ...
  n=nrow(Data)
  ############use Shortreed_et_al-2017 codes from supplementary#########
  ...
  y.form = formula(paste(outcome.varname, "~",paste(c(treat.varname, varList),collapse="+")))
  lm.Y = lm(y.form, data=Data)
  betaXY = coef(lm.Y)[varList] 
  ...
  coeff_XA = as.data.frame(matrix(NA,nrow=length(varList),ncol=length(lambda_vec)))
  names(coeff_XA) = names(lambda_vec)
  rownames(coeff_XA) = varList
  ...
  w.full.form = formula(paste(treat.varname, "~",paste(varList,collapse="+")))
  for( lil in names(lambda_vec) ){
    ...
    coeff_XA[varList,lil] = coef(logit_oal)[varList]
    # create inverse probability of treatment weights
    Data[,paste("w",lil,sep="")] = create_weights(fp=Data[,paste("f.pA",lil,sep="")],fA=Data[, treat.varname])
    # estimate weighted absolute mean different over all covaraites using this lambda to generate weights
    wAMD_vec[lil] = wAMD_function(DataM=Data,varlist=varList, trt.var=treat.varname,
                                  wgt=paste("w",lil,sep=""), beta=betaXY)$wAMD
    # save ATE estimate for this lambda value
    ATE[lil] = ATE_est(fY=Data[, outcome.varname],fw=Data[,paste("w",lil,sep="")],fA=Data[, treat.varname])
  ...
  selectedVar=varList[(which(abs(coeff_XA[,tt])>threshold))]
...
####################traditional adaptive lasso########################
adapLassoT=function(Data, varList, treat.varname, threshold=10^-8) {
  ...
  # estimate outcome model
  y.form = formula(paste(treat.varname, "~",paste(c(varList),collapse="+")))
  lm.Y = glm(y.form, data=Data, family = "binomial")
  betaXY = coef(lm.Y)[varList] 
