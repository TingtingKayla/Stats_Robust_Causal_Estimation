...
###outcome adaptive lasso on propensity model
###dataOR-original dataset
###data-bootstrap sample

#dataOR=simdat; data=simdat; varList=varList; propenVarList=propenVarList;
#outcomeVarList0=outcomeVarList0; outcomeVarList1=outcomeVarList1;
#treat.varname=treat.varname; outcome.varname=outcome.varname; splineTerm = splineTerm; method=Method


pencompLassoOutcome=function(dataOR, data, varList, propenVarList, outcomeVarList0, outcomeVarList1, treat.varname, outcome.varname) {
  tryCatch ( 
    {
      ...
      if ( length(propenVarList)==0 ){
        propenVarList=outcomeAdapLasso(Data=data, varList=varList, treat.varname = treat.varname, outcome.varname = outcome.varname, threshold=10^-8)
      } 
